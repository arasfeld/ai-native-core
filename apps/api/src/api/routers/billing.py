...
async def _get_tenant(pool, tenant_id: int) -> dict:
...
async def _get_monthly_usage(pool, tenant_id: int) -> int:
...
async def get_plan(request: Request, current_user: CurrentUser) -> PlanInfo:
    ...
    tenant = await _get_tenant(pool, current_user.tenant_id)
    used = await _get_monthly_usage(pool, current_user.tenant_id)
...
async def create_checkout(request: Request, current_user: CurrentUser) -> CheckoutResponse:
    ...
    log.info("billing.checkout.created", tenant_id=current_user.tenant_id, session_id=session.id)
...
async def create_portal(request: Request, current_user: CurrentUser) -> PortalResponse:
    ...
    log.info("billing.portal.created", tenant_id=current_user.tenant_id)
