...
function UsageBar({ used, limit }: { used: number; limit: number }) {
  ...
  return (
    <div className="space-y-1">
      ...
      <p className="text-xs text-muted-foreground">{pct}% of monthly budget used</p>
    </div>
  );
}
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-8 p-8">
        ...
        <div className="space-y-6 rounded-xl border p-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-muted-foreground">
              {plan.plan === "pro" ? "2,000,000 tokens/month" : "100,000 tokens/month"}
            </span>
          </div>

          {/* Usage bar */}
          <UsageBar used={plan.tokens_used_this_month} limit={plan.token_limit} />
      ...
      <div className="grid grid-cols-2 gap-4">
        {[
          { name: "Free", tokens: "100,000", price: "$0/mo", features: ["1 user", "100k tokens/month", "Community support"] },
          { name: "Pro", tokens: "2,000,000", price: "$20/mo", features: ["1 user", "2M tokens/month", "Priority support", "Longer memory"] },
        ].map((tier) => (
          <div
            key={tier.name}
