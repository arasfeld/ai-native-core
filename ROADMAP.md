...
| 133 | **Token → dollar cost computation** | ✅ | Migration `0019_session_usage_cost.sql` extends `session_token_usage` with `provider`, `model`, `input_tokens`, `output_tokens`, `cost_usd`, and adds optional `tenants.cost_limit_usd`. Providers now stamp `provider`/`model` onto every `Usage` event; `ChatService` resolves the dollar cost through `app.state.pricing.compute_cost` after each turn (NULL when the provider doesn't expose usage, e.g. Ollama). `TenantMonthlyBudget` switches into dollar mode when `cost_limit_usd` is set on the tenant — `SessionRepository.check_budget` picks the path. Guests stay token-capped (10k). `GET /billing/plan` returns the month-to-date cost alongside the dollar cap |
